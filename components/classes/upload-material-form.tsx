...
import { uploadMaterialAction } from "@/app/classes/[classId]/actions";
...
export function UploadMaterialForm({ classId }: { classId: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs uppercase tracking-[0.25em] text-slate-500">Materials</p>
      <h2 className="mt-2 text-xl font-semibold text-ink">Upload study material</h2>
      <p className="mt-3 text-sm leading-6 text-slate-600">
        This stores the file in Supabase Storage and adds it to the class resource list.
      </p>
      ...
      <label className="mt-5 block">
        ...
        <input
          required
          name="title"
          placeholder="Unit 3 revision sheet"
      ...
      <label className="mt-4 block">
        ...
        <input
          ...
          name="file"
          type="file"
          className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-ink file:mr-4 file:rounded-full file:border-0 file:bg-ink file:px-4 file:py-2 file:text-sm file:font-medium file:text-white"
        />
      </label>
